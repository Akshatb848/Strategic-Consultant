...
class QualityGate:
    ...
    async def validate(self, brief: StrategicBriefV4, retry_count: int = 0) -> QualityReport:
        ...
        overall_grade = self._grade(checks, mece_score, citation_density_score, internal_consistency_score)
    ...
    def _grade(
        ...
        mean_score = mean([mece_score, citation_density_score, internal_consistency_score])
        if mean_score >= 0.85 and all(check.passed for check in checks):
            return "A"
        if mean_score >= 0.72:
            return "B"
        return "C"
