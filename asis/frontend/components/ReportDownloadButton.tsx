...
export function ReportDownloadButton({ analysisId, theme, disabled = false, disabledReason }: ReportDownloadButtonProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleDownload = async () => {
    if (disabled) {
      setError(disabledReason || "PDF export is blocked for this report.");
      return;
    }
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="flex flex-col items-end gap-3">
      ...
      >
        ...
        {disabled ? "PDF Export Blocked" : "Download Enterprise Report (PDF)"}
      </button>
      {error ? (
